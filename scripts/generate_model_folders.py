...
def generate(output_root: Path) -> None:
    ...
    for i, model_id in enumerate(model_ids, 1):
        ...
        (folder / "server.py").write_text(server_tpl, encoding="utf-8")
        (folder / "start.sh").write_text(start_sh, encoding="utf-8")
        (folder / "start.bat").write_text(start_bat, encoding="utf-8")
        (folder / "adapter.py").write_text(build_adapter(model_id), encoding="utf-8")

        cls = get_adapter_class(model_id)
        (folder / "requirements.txt").write_text(build_requirements(cls.category), encoding="utf-8")

        weights_dir = folder / "weights"
        weights_dir.mkdir(exist_ok=True)  # 保留用户已放置的权重
        readme_path = weights_dir / "README.txt"
        if not readme_path.exists():
            readme_path.write_text(weights_readme, encoding="utf-8")
